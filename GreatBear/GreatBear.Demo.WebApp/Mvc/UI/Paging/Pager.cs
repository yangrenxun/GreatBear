using GreatBear.Demo.WebApp.Mvc.Extensions;
    ...
    public partial class Pager : IHtmlContent
    {
        ...
        public virtual string GenerateHtmlString()
        {
            ...
            if (showTotalSummary && (model.TotalPages > 0))
            {
                ...
                links.Append(string.Format("第{0}/{1}页(共{2}条记录)", model.PageIndex + 1, model.TotalPages, model.TotalItems));
                links.Append("</li>");
            }
            if (showPagerItems && (model.TotalPages > 1))
            {
                if (showFirst)
                {
                    ...
                    {
                        links.Append(CreatePageLink(1, "首页", firstPageCssClass));
                    }
                }
                if (showPrevious)
                {
                    ...
                    {
                        links.Append(CreatePageLink(model.PageIndex, "上一页", previousPageCssClass));
                    }
                }
                ...
                if (showNext)
                {
                    ...
                    {
                        links.Append(CreatePageLink(model.PageIndex + 2, "下一页", nextPageCssClass));
                    }
                }
                if (showLast)
                {
                    ...
                    {
                        links.Append(CreatePageLink(model.TotalPages, "尾页", lastPageCssClass));
                    }
                }
            }
        ...
        protected virtual string CreateDefaultUrl(int pageNumber)
        {
            ...
            var url = GetThisPageUrl(false);
            foreach (var routeValue in routeValues)
            {
                url = ModifyQueryString(url, routeValue.Key, routeValue.Value?.ToString());
            }
            if (renderEmptyParameters && parametersWithEmptyValues.Any())
            {
                foreach (var key in parametersWithEmptyValues)
                {
                    url = ModifyQueryString(url, key);
                }
            }
            return url;
        }

        private string GetThisPageUrl(bool includeQueryString, bool? useSsl = null, bool lowercaseUrl = false)
        {

            //add local path to the URL
            var pageUrl = $"{ViewContext.HttpContext.Request.Scheme}://{ViewContext.HttpContext.Request.Host}{ViewContext.HttpContext.Request.Path}";

            //add query string to the URL
            if (includeQueryString)
                pageUrl = $"{pageUrl}{ViewContext.HttpContext.Request.QueryString}";
        ...
        private string ModifyQueryString(string url, string key, params string[] values)
        {
            ...
            url = $"{uri.AbsolutePath}{queryBuilder.ToQueryString()}{uri.Fragment}";
